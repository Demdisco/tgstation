...
/mob/living/simple_animal/hostile/floor_cluwne
	...
	loot = list(/obj/item/soap/deluxe)
	wander = FALSE
	minimum_distance = 2
	move_to_delay = 1
...
/mob/living/simple_animal/hostile/floor_cluwne/Life()
	...
	if(is_type_in_typecache(src.loc, invalid_area_typecache) || !is_station_level(z))
	...
	if(prob(5))//checks roughly every 20 ticks
		if(current_victim.stat == DEAD || is_type_in_typecache(T, invalid_area_typecache) || !is_station_level(current_victim.z))
			if(!Found_You())
				Acquire_Victim()

	if(get_dist(src, current_victim) > 9 && !manifested &&  !is_type_in_typecache(T, invalid_area_typecache))//if cluwne gets stuck he just teleports
		do_teleport(src, T)
...
/mob/living/simple_animal/hostile/floor_cluwne/Goto(target, delay, minimum_distance)
	if(!manifested && !is_type_in_typecache(current_victim.loc, invalid_area_typecache) && is_station_level(current_victim.z))
...
/mob/living/simple_animal/hostile/floor_cluwne/proc/Acquire_Victim(specific)
	for(var/I in GLOB.player_list)//better than a potential recursive loop
		...
		if(specific)
			H = specific
			if(H.stat != DEAD && H.has_dna() && !is_type_in_typecache(H.loc, invalid_area_typecache) && is_station_level(H.z))
				return target = current_victim

		if(H && ishuman(H) && H.stat != DEAD && H != current_victim && H.has_dna() && !is_type_in_typecache(H.loc, invalid_area_typecache) && is_station_level(H.z))
			current_victim = H
			interest = 0
			stage = STAGE_HAUNT
			return target = current_victim
